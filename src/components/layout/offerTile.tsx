    ...
    return (
        <div className="relative rounded-xl w-fit shadow-[0_8px_40px_0_rgba(164,167,174,0.12)]">
                ...
                <CarouselContent>
                    {offer.images.map((image, index) => (
                        <CarouselItem key={index}>
                            <Image className="rounded-t-xl" src={image.urlNormal} alt={`offer image ${index + 1}`} width={424} height={239} />
                            </CarouselItem>
                    ))}
                </CarouselContent>
            ...
            <div className="p-4">
                ...
                <h2 className="font-black text-2xl my-4 text-green-primary">{formatPrice(offer.price)}</h2>
            </div>
        </div>
    )
}
