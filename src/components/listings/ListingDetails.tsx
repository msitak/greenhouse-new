...
export default function ListingDetails({ listing, propertyType }: ListingDetailsProps) {
  ...
  const BooleanRow = ({ label, value, hideOnFalse = false }: { label: string; value: boolean | null | undefined; hideOnFalse?: boolean }) => {
  ...
  return (
    <div className='flex flex-col text-sm w-full'>
      ...
      <DetailRow label='Powierzchnia' value={listing.area ? `${listing.area} m²` : null} />
      ...
      {(propertyType === 'mieszkanie' || propertyType === 'dom') && (
        <>
          ...
          {propertyType === 'dom' && details.lotArea && (
            <DetailRow label='Powierzchnia działki' value={`${details.lotArea} m²`} />
          )}
      ...
      {propertyType === 'dzialka' && (
        <>
          ...
          <DetailRow 
            ...
            value={details.possibleDevelopmentPercent ? `${details.possibleDevelopmentPercent}%` : null} 
          />
          <DetailRow 
            label='Maksymalna wysokość zabudowy' 
            value={details.overallHeight ? `${details.overallHeight} metrów` : null} 
          />
          <DetailRow label='Forma własności' value={details.groundOwnershipType} />
          <DetailRow label='Media & uzbrojenie' value={(() => {
            const media = [];
            if (details.electricityStatus) media.push('Prąd');
            if (details.gasStatus && details.gasStatus.toLowerCase() !== 'no') media.push('Gaz');
            if (details.waterTypeList && Array.isArray(details.waterTypeList) && details.waterTypeList.length > 0) {
              media.push('Woda');
            }
            if (details.sewerageTypeList && Array.isArray(details.sewerageTypeList) && details.sewerageTypeList.length > 0) {
              media.push('Kanalizacja');
            }
            return media.length > 0 ? media.join(', ') : null;
          })()} />
          <BooleanRow label='W granicach miasta' value={details.urbanCo} hideOnFalse />
          <BooleanRow label='Pozwolenie na budowę' value={details.activeBuildingPermit} hideOnFalse />
          <BooleanRow label='Wydane warunki zabudowy' value={details.issuedBuildingConditions} hideOnFalse />
          <BooleanRow label='Możliwość podziału' value={details.dividingPossibility} hideOnFalse />
        </>
      )}
      ...
      <DetailRow label='Data dodania' value={formatDate(listing.createdAtSystem)} />
      {details.vacantFromDate && (
        <DetailRow label='Dostępne od' value={formatDate(details.vacantFromDate)} />
      )}
      <ArrayRow label='Komunikacja' value={details.communicationList} />
      <ArrayRow label='Dostępne w okolicy' value={details.availableNeighborhoodList} />
      <BooleanRow label='Oferta zewnętrzna' value={details.ownListing === false} hideOnFalse />
