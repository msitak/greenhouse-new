...
describe('GET /api/listing (rozbudowane testy listy ofert)', () => {
  ...
  it('POW-001: powinien zwrócić domyślną listę ofert z poprawną paginacją i strukturą', async () => {
    ...
    if (response.body.pagination) {
      ...
      expect(response.body.pagination.totalPages).toBe(
        Math.ceil(response.body.pagination.totalItems / 10)
      );
    }
  ...
  describe('Sortowanie', () => {
    ...
    sortableFields.forEach(sortInfo => {
      ['asc', 'desc'].forEach(order => {
        it(`SORT-001: powinien sortować po ${sortInfo.field} ${order}`, async () => {
          const response = await request(API_BASE_URL).get(
            `/listing?sortBy=${sortInfo.field}&order=${order}&limit=5`
          );
          expect(response.status).toBe(200);
          ...
          if (items.length > 1) {
            for (let i = 0; i < items.length - 1; i++) {
              ...
              if (sortInfo.type === 'date') {
                // Dla dat konwertujemy na obiekty Date
                const date1 = new Date(val1);
                const date2 = new Date(val2);
                if (order === 'asc') {
                  expect(date1.getTime()).toBeLessThanOrEqual(date2.getTime());
                } else {
                  expect(date1.getTime()).toBeGreaterThanOrEqual(
                    date2.getTime()
                  );
                }
              } else {
                // Dla stringów i liczb
                if (order === 'asc') {
    ...
    it('SORT-002: powinien użyć domyślnego sortowania, gdy sortBy jest niepoprawne', async () => {
      const responseDefault =
        await request(API_BASE_URL).get('/listing?limit=5'); // createdAtSystem desc
      const responseInvalid = await request(API_BASE_URL).get(
        '/listing?sortBy=invalidField&limit=5'
      );
      ...
      if (
        responseDefault.body.data.length > 0 &&
        responseInvalid.body.data.length > 0
      ) {
        expect(responseInvalid.body.data[0].id).toEqual(
          responseDefault.body.data[0].id
        );
      }
  ...
  describe('Filtrowanie', () => {
    ...
    it('FILT-006: powinien zwrócić pustą listę, gdy filtry nie pasują do żadnej oferty', async () => {
      const response = await request(API_BASE_URL).get(
        '/listing?city=MiastoKtoregoNieMaNaPewno&priceMin=999999999'
      );
      expect(response.status).toBe(200);
      expect(response.body.data).toEqual([]);
      if (response.body.pagination) {
  ...
  describe('Przypadki brzegowe', () => {
    ...
    it('EDGE-002: powinien zignorować nieznane parametry zapytania', async () => {
      ...
      const responseWithUnknown = await request(API_BASE_URL).get(
        '/listing?limit=2&nieznanyParametr=wartosc'
      );
      ...
      expect(responseWithUnknown.body.pagination).toEqual(
        responseValid.body.pagination
      );
    });
  });
});
